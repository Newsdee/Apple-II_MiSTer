...
module composite_decoder_v5a #(
	...
	parameter SPC = 4,   // Apple II: 14.318 MHz / (4 x 3.579545 MHz)
	// Phase the free-running subcarrier accumulator is re-anchored to at each
	// LOCKED recovered line start (sync leading edge on a 912-sample line;
	// the hpos gate in the phase always_ff). 2^24 per subcarrier cycle.
	// This is the value the accumulator naturally holds at the locked line
	// start in steady state - the natural PRE-update value: 0x000000,
	// measured at every locked edge (hpos 911, tb hcnt 131) on plain and
	// colour-zone lines, fields 0/1/12/16 (tb_v5_regress lsmon, no-anchor
	// build, 14/14 samples). Re-asserting it on a stable line is a no-op;
	// it recovers the hue after a machine_ce stall + source-position jump
	// (F5 save/load), where the frozen accumulator would otherwise stay
	// offset - the decoder-side mirror of apple_composite's
	// BURST_PHASE_HS_FALL re-anchor, which uses the same
	// natural-steady-state-value rule.
	// NB: the re-anchor assignment itself is PHASE_SYNC_RISE + PHASE_INC (the
	// natural post-update value), because the re-anchor branch skips that
	// cycle's advance (see the phase always_ff). Assigning PHASE_SYNC_RISE
	// alone drops one increment per line and the phase at sync rise drifts by
	// -PHASE_INC every line.
	// CAVEAT: this constant must be measured at LOCKED edges only. The first
	// ~12 line_start events after power-on are spurious (tip convergence,
	// 4 samples apart) and the phase there is 2 samples (180 deg) off the
	// locked value: an earlier build anchored to the transient value
	// 0x800000 and silently shifted the steady-state phase grid by 180 deg
	// (invisible in decoded hue - the burst-referenced decode cancels any
	// whole-90-degree offset - but a grid shift is not a no-op).
	parameter PHASE_SYNC_RISE = 24'h000000,
...
localparam [10:0] BURST_GATE  = 11'(tenths_us(54));   // Apple: burst measured from here (77)
localparam [10:0] CLAMP_GATE  = 11'(tenths_us(99));   // Apple: black measured from here (141)
...
logic [15:0] linebuf_rd;   // port B registered read (replaces dpram ram_rd)
...
// Line buffer (previous line) for the comb. Replaces v5's dpram instance: the
// project's dpram has a different interface (addr_width_g/enable_* vs
// widthad_a/byteena_*), so use an inferred M10K array matching the current
// decoder's proven pattern. Port A writes c16 at hcnt_d (gated by ce); port B
// is an ungated REGISTERED read at comb_rd (= (hcnt+1) mod line_len), one
// cycle behind hcnt, so read and write addresses differ by ~1-2 samples and
// never collide (the dpram's NEW_DATA read-during-write is moot). The buffer
// is 2^HCNT_W deep and comb_rd wraps at the self-measured line_len (< 2^W), so
// every address stays in range. (Assumes HCNT_W=10: Apple II 912-sample line.)
logic [15:0] linebuf_mem [0:1023] /* synthesis ramstyle = "M10K" */;
always_ff @(posedge clk) begin
	if (ce) linebuf_mem[hcnt_d] <= $unsigned(c16);   // port A write (wren_a=ce)
	linebuf_rd <= linebuf_mem[comb_rd];              // port B read (ungated, registered)
end
...
always_ff @(posedge clk) if (reset) begin
	...
	hcnt_d <= hcnt;
	prev   <= (lines_seen == 2'd3) ? $signed(linebuf_rd) : c16;
...
always_ff @(posedge clk) if (reset) begin
	phase <= 24'd0;
	phase_rem <= '0;
end else if (line_start && (hpos == 10'd911)) begin
	// F5 re-anchor: re-align hue at each LOCKED line start. hcnt is the
	// recovered line counter (reset to 0 at every line_start), so hpos == 911
	// means the previous recovered line ran exactly 912 samples - a locked,
	// 912-aligned edge. The gate is required: power-on spurious line_start
	// events (tip convergence) carry partial-line hpos values; re-anchoring
	// there would leave the grid offset by (edge spacing mod 4) samples and
	// corrupt the first locked line's pre-edge phase. A slipped edge in real
	// operation (hpos != 911) simply skips the re-anchor for that line - the
	// phase keeps running naturally and the next locked line re-anchors - so
	// the re-anchor never pins the grid to the wrong slot.
	// The branch skips this cycle's advance, so it must reproduce the natural
	// post-update value (PHASE_SYNC_RISE + PHASE_INC = 0x400000, the natural
	// line-start post value); assigning PHASE_SYNC_RISE alone loses one
	// increment per line -> phase@sync drifts by -PHASE_INC every line.
	// phase_rem is left at its natural value (with SPC=4 it stays identically
	// 0: PHASE_FRAC = 2^24 % 4 = 0, so it cannot desync the evolution).
	phase <= PHASE_SYNC_RISE + PHASE_INC[23:0];
end else if (ce) begin
