...
**SD-persistence mechanics (verified in miosd, 2026-09-10):** miosd's `process_ss` maps 4 slots of `ss_size` (from `SSbase:size`) at the SS base, zeros them, writes `0xFFFFFFFF` into the first DWORD of each slot, loads `<game>.ssN` files if present, and polls the first DWORD every second as a change counter. It only activates for cores whose file option carries the `S` flag (e.g. NES `FS,NESFDSNSF;`) - our `S0`/`S1`/`S2` storage options do not, so the interim core is safe with the SS region advertised. When persistence is implemented: (a) the slot stride must equal `ss_size` (miostd always uses `ss_size` per slot; our 512 KiB slots would need `SS3E000000:80000`, or the slots grow to 2 MiB to match the NES `:200000` precedent), and (b) the manager must write a changing counter into the slot's first DWORD on every save (the current header0 magic is static and would never trigger a write-back).
