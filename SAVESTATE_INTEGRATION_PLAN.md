...
- **dsk->woz for the woz build (design 2026-09-11):** miosd cannot see core build dates (RBF is opaque) - the variant marker is the confstr core name (field 0), the stock upstream discriminator. Both builds are named "Apple-II" and both hit `a2_core` (also matches TK2000), so a dsk->woz hook keyed on name+.dsk would hijack the standard build's working `SD_TYPE_A2`/dsk2nib flow - the backward-compat break to avoid. Fix (revised 2026-09-11 - the user REJECTED a core rename: a new confstr name loses the existing miosd configs/savestates keyed by name): a RUNTIME capability bit - the woz build drives a spare `status` bit (e.g. bit 9) through the core->miOSD `status_in`/`status_set` path (the shipping NES `increaseSSHeaderCount` precedent drives a status bit exactly this way); the miosd patch = `if (user_io_status_get("[9]"))` + a mount hook beside `iigs_mount` at the v260823 classification site (user_io.cpp ~2180) reusing the conversion machinery ALREADY in v260823 (PR #1011 `5ee7f9e` 2025-07-30, PR #1216 `f8a198e` 2026-06-15 - both ancestors of `dfb4791`: iigs_fmt/dsk2nib_lib/in-memory WOZ buffer/write-back). Upstream's locked decisions fit: native .woz byte-for-byte (copy protection), on-the-fly translation, no temp files. Status-bit gating is immune to confstr edits and to stock-miosd risk (stock firmware never reads the bit); old builds and the standard build stay byte-for-byte unchanged (exact hps_io status-merge wiring to be verified against the NES precedent at implementation time). Confstr-token alternative (zero RTL): a capability token in field 1 - but parse_config (v260823 `user_io.cpp:700`) advances ONLY via commas, so an unknown token as the LAST comma-separated element INFINITE-LOOPS stock miosd at boot (traced: the current field terminates only because the trailing UART digits drive p to end-of-string); only a MID-field token (e.g. `SS3E000000:200000,D2W,UART19200:...`) is safely skipped - fragile, avoid. Note: with the name unchanged, standard and woz builds share `savestates/Apple-II/`; if the .ss layouts ever diverge, bump the manager's version byte so the load check rejects foreign states cleanly.
