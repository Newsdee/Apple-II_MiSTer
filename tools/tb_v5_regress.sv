...
//   3. PHASE_AT_SYNC_RISE: u_v5.phase at LOCKED v5 line starts (hpos 911)
//      is constant over 6 lines at the natural value 0x000000 -> the
//      re-anchor constant for the F5-LOAD fix (A6) is a steady-state no-op.
...
module tb_v5_regress;
  ...
  // Monitor 3: PHASE_AT_SYNC_RISE - u_v5.phase at LOCKED line starts (v5's
  // line_start on a 912-aligned edge, hpos == 911, tb hcnt 131) over 6
  // consecutive lines. Must equal the re-anchor constant (PHASE_SYNC_RISE)
  // and be constant across lines. Sampling only locked edges is required:
  // the first ~12 line_start events after reset are spurious (power-on tip
  // convergence, tb hcnt 25..69, 4 samples apart) and their phase samples
  // are trivially constant mod 2^24 (4*PHASE_INC == 0), so the old
  // first-6-events monitor measured a power-on transient, not the
  // steady-state grid the re-anchor constant is defined at.
  reg [23:0] phase_at_sync [0:5];
  integer    phase_n = 0;
  always @(posedge CLK) begin
    if (v5_line_start && (u_v5.hpos == 10'd911) && phase_n < 6) begin
  ...
  initial begin
    ...
    // ---- Check 3: PHASE_AT_SYNC_RISE at locked edges (re-anchor constant) ----
    // Expected value 24'h000000 = the natural locked line-start phase
    // (PHASE_SYNC_RISE), measured on the no-anchor build. The value gate is
    // required in addition to constancy: a re-anchor constant that shifts
    // the steady-state grid (e.g. the transient-measured 0x800000) is still
    // constant line-to-line and would pass a constancy-only check.
    $display("=== [3] PHASE_AT_SYNC_RISE (v5.phase at LOCKED sync rise, 6 lines) ===");
    begin : phase_block
      integer i;
      logic constant = 1'b1;
      for (i = 1; i < 6; i++)
      ...
      $display("PHASE_AT_SYNC_RISE = 24'h%06h  (top-8 %08h)  constant=%0b  expected=24'h000000",
               phase_at_sync[0], phase_at_sync[0][23:16], constant);
      if (constant && (phase_at_sync[0] == 24'h000000))
        $display("PASS (locked-edge phase stable at the natural value -> re-anchor is a steady-state no-op)");
      else begin
        $display("FAIL (locked-edge phase not stable at 0x000000 -> re-anchor constant wrong or grid shifted)");
        fails = fails + 1;
      end
    end
