...
module composite_decoder_v5a #(
	...
	// recovered line start (sync leading edge). 2^24 per subcarrier cycle, so
	// for a 912-sample line (912 % 4 == 0) this is the value the accumulator
	// naturally holds at line start in steady state (measured by the Stage 1
	// TB, tb_v5_regress). Re-asserting it on a stable line is a no-op; it
	// recovers the hue after a machine_ce stall + source-position jump (F5
	// save/load), where the frozen accumulator would otherwise stay offset.
	// NB: this is the natural PRE-update value; the re-anchor assignment
	// itself is PHASE_SYNC_RISE + PHASE_INC (the natural post-update value),
	// because the re-anchor branch skips that cycle's advance (see the phase
	// always_ff). Assigning PHASE_SYNC_RISE alone drops one increment per
	// line and the phase at sync rise drifts by -PHASE_INC every line.
	parameter PHASE_SYNC_RISE = 24'h800000,
